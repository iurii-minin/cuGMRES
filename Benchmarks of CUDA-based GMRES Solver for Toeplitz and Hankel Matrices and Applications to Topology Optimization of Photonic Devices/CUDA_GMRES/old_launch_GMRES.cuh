...
{		
	...
	for (unsigned int repetition_i = max_st; repetition_i < max_en; repetition_i ++)
	{
		...
		for (unsigned int maxiter = 3; maxiter < 50; maxiter ++)
		{
			...
			for (unsigned int N = 2048L; N < 5000; N *= 2) //Characteristic size of square matrix
			{
				fprintf(stderr, "%i\n", N);
				...
				cuComplex *h_analytical_solution = (cuComplex *)malloc(N * N * sizeof(cuComplex));
			...
			//	unsigned int jj = 0;
				...
				if (analytical_solution_file.is_open())
				{
					...
					while ( getline (analytical_solution_file, line) )
					{
						...
						in_string_stream >> h_analytical_solution[index].x >> h_analytical_solution[index].y;
				...
				{
					fprintf(stderr, "Unable to open analytical solution file");
					exit(1);
				}
				...
				cublascall(cublasScnrm2(handle, N * N,
							(const cuComplex *)dev_analytical_solution, 1, (float  *)&h_norm_analytical_solution));
	...
	//			saveGPUrealtxt_C(dev_solution, "/gpfs/gpfs0/i.minin/input/solution.txt", N * N);

				free((cuComplex *)h_analytical_solution);
